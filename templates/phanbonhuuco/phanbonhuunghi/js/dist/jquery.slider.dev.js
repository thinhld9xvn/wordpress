...
var slider = {
  ...
  bxslider_objects_instances: [],
  // lưu trữ các mảng id slider => instance nào
  bxslider_objects_pagethumb_instances: [],
  // lưu trữ các mảng id pagethumb slider => instance nào
  bxslider_public_options: {},
  // lưu trữ các options cho slider có mode = carousel
  ...
  initBxSlider: function initBxSlider() {
    ...
    jQuery('.bxslider').each(function (index, elem) {
      var $slider = jQuery(elem),
        ...
        slideMargin: $slider.attr('data-bxslider-margin') !== undefined ? parseInt($slider.attr('data-bxslider-margin')) : 20,
        pager: false,
      ...
      if (slider.bxslider_instances[count] === undefined) {
        slider.bxslider_instances[count] = $slider.bxSlider(options); //console.log( slider.bxslider_instances[count] );
      ...
      if ($slider.data('bxslider-object') === 'tab') {
        $slider.closest('.singleTabBox-cn-wrapper').find('.bx-controls-direction > a').unbind('click').bind('click', {
          $slider: $slider,
          options: options,
          instance: slider.bxslider_instances[count]
  ...
  initBxPageThumbSlider: function initBxPageThumbSlider() {
    var $bx_pagethumb = jQuery('.bx-pagethumb'),
        count = 0;
    ...
    if ($bx_pagethumb.length > 0) {
      $bx_pagethumb.each(function (index, elem) {
        var w = jQuery(window).width(),
            ...
            numsShow = $slider.data('numshow') !== undefined ? parseInt($slider.data('numshow')) : 3,
            options = {
          pager: false,
          minSlides: numsShow,
          maxSlides: numsShow,
        ...
        if (992 <= w && w <= 1250) {
          ...
          options.slideMargin = 0;
          options.minSlides = options.maxSlides = numsShow;
        } else if (w < 600) {
          options.slideMargin = 0;
          options.minSlides = options.maxSlides = 2;
        }
  ...
  onBxSliderAction: function onBxSliderAction($slideElement, oldIndex, newIndex, action) {
    ...
    if ($slider_pagethumb.length > 0) {
      var $target = $slider_pagethumb.find('.pageitem').find('a');
      $target.removeClass('active').filter('a[data-slide-index=' + newIndex + ']').addClass('active');
    ...
    if ($slider.data('bxslider-auto') === undefined || $slider.data('bxslider-auto') !== undefined && JSON.parse($slider.data('bxslider-auto')) === true) {
      slider_instance.startAuto();
    }
  },
  onBxSliderBefore: function onBxSliderBefore($slideElement, oldIndex, newIndex) {
    slider.onBxSliderAction($slideElement, oldIndex, newIndex, '');
  },
  onBxSliderNext: function onBxSliderNext($slideElement, oldIndex, newIndex) {
    slider.onBxSliderAction($slideElement, oldIndex, newIndex, 'nextSlide');
  },
  onBxSliderPrev: function onBxSliderPrev($slideElement, oldIndex, newIndex) {
    ...
    if ($slider.data('bxslider-object') === 'tab') {
      setTimeout(function () {
        if (mode === 'horizontal') {
          ...
          if (total_twidth > 0) {
            $slider.closest('.bx-viewport').outerWidth(total_twidth + 2);
          }

          if (slides_length > numSlidesShow) {
            $slider.closest('.bxslider-cn-wrapper').find('.bx-controls-direction > a').removeClass('disabled');
          }
  ...
  onBxsliderTabAction: function onBxsliderTabAction(e) {
    e.preventDefault();
    var $slider = e.data.$slider,
        instance = e.data.instance,
        options = e.data.options,
        $bx_controls_direction = $slider.closest('.bxslider-cn-wrapper').find('.bx-controls-direction > a'),
    ...
    if (action === 'nextSlide' || action === 'prevSlide') {
      var numSlidesShow = options.minSlides,
          ...
          total_index = 0,
          slides_length = $slider.find('> *').length;
  ...
  onBxPageThumbItemClick: function onBxPageThumbItemClick(e) {
    e.preventDefault();
  ...
  onBxCustomControlsClick: function onBxCustomControlsClick(e) {
    e.preventDefault();
    var $obj = jQuery(this),
        type = $obj.parent().data('bxsliderType'),
        $controls = $obj.closest('.bxslider-cn-wrapper').find('.bx-wrapper').find('.bx-controls').find('a'),
        action = $obj.attr('class');

    if (type === 'bx-pagethumb') {
      var slider_target_id = jQuery(this).closest('.bxslider-cn-wrapper').find('.bx-pagethumb').attr('data-bxslider-target'),
          instance_index = slider.bxslider_objects_instances[slider_target_id],
