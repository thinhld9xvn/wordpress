function responsiveBorderMainSlider() {
    ...
    const BORDER_OFFSET_SP = 30,
          BORDER_LEFT_OFFSET_SP = 30,
          BORDER_OFFSET_HEIGHT = 2;

    const $slider_items = jQuery('.slider-main-item');

    $slider_items.each(function(i, e) {

        const $slider_item = jQuery(e),
              ...
              border_bottom_offset_width = container_width - last_elem_width - BORDER_OFFSET_SP;

        //console.log($last_elem, $last_elem.width());
...
function responsiveSlider() {
    ...
    const performRespMainSlider = () => {

        const pos = ( jQuery(window).width() - jQuery('.container').width() ) / 4;
    ...
    const performRespPortolifoFeedsSlider = () => {

        const $container = jQuery('.bxslider-portfoliofeeds-cn-wrapper').closest('.container'), 
...
function responsiveRespListBox() {
    ...
    const performRespBgList = function() {
        ...
        if ( jQuery(window).width() <= 768 ) {
            ...
            const $items = jQuery('.grid-resp-listbox .item');
            ...
            $items.each(function(i, e) {
                ...
                const $background = $item.find('.bg-thumbnail'),
                ...
                //console.log(background_offset_height);
