				...
				<div class="search-filters type-<?php echo esc_attr( $type->get_slug() ) ?> type-id-<?php echo absint( $type->get_id() ) ?>">
					<div class="light-forms filter-wrapper">
						...
						<?php foreach ( (array) $type->get_advanced_filters() as $filter ) {
							...
							$label = str_format( $filter->get_label(), 'lowercase' );

							/*if ( 0 === strpos( $label, _LABEL_WHAT_ARE_YOU_LOOKING_FOR ) || 
								 0 === strpos( $label, _LABEL_CATEGORY ) ) continue;*/	
								 
							if ( $label === _LABEL_SALARY ) :

								print_salary_filter_box($filter, _LABEL_LOCATION, _LABEL_ONCHANGE);
