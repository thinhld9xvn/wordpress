...
jQuery( function($) {
	var accordion = {
		...
		on_accordion_stopSortupdate: function( event, ui ) { 
			...
			$('.metabox-accordion-item:not(.accordion-template-item)').each(function (accordion_item, i) {
				$fields_has_name = $(accordion_item).find('*[name]');
				$fields_has_name.each(function (field, k) {
