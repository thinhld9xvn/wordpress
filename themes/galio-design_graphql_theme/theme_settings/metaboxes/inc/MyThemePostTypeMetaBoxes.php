<?php   
    class MyThemePostTypeMetaBoxes {
        ...
        **/
                ...
                if ( isset( $metabox['where_show_on'] ) && 
                    ...
                    if ( $arr_cat_slug ) :                                              
                        ...
                        if ( 'page' === $metabox['where_show_on'] ) :
                            ...
                            $page_ids = array_map('intval', $metabox['page_id']);
                            if ( $page_template ) :
        ...
        public static function get_terms($tax, $parent) {
            $args = array(
                'taxonomy' => $tax,
                'hide_empty' => false,
                'parent' => $parent
...
<?php   }
        public static function print_select_field_options_data($field_options, $field_value) { ?>
            <option value="-1">--- Mời chọn một mục ---</option>
            <?php while ( $option_name = current( $field_options ) ) : ?>
                <option value="<?php echo key( $field_options ) ?>" 
                        <?php selected( $field_value, key( $field_options ) ); ?>>
        ...
        public static function print_page_field_select_options($field_value) {
            $args = array(
                'hide_empty' => '0',
                'order' => 'asc',
                'orderby' => 'id'
           ...
           <?php foreach ( $pages as $page ) :  ?>
                <option value="<?= $page->ID ?>" 
                        <?= $page->ID == $field_value ? 'selected="selected"' : '' ?>>
                    <?= $page->post_title  ?>
                </option>
           <?php endforeach;
        }
           ...
           <div id="metabox_field_<?php echo $field['id'] ?>" 
                ...
                <div class="field mtop10">
                    <select id="<?php echo $field['id']; ?>" 
                        ...
                        <?php 
                            if ( $field['options'] ) :
                                self::print_select_field_options_data($field['options'], $field_value);
                            elseif ( $field['data'] ) :
                                if ( $field['data'] === 'pages' ) :
                                    self::print_page_field_select_options($field_value);
                                else :
                                    if ( $field['data']['query_tax'] ) :
                                        $terms = self::get_terms($field['data']['taxonomy'], $field['data']['parent']);
                                        $tax_options = array();
                                        foreach ( $terms as $key => $term ) :
                                            $tax_options[$term->slug] = $term->name;
                                        endforeach;
                                        self::print_select_field_options_data($tax_options, $field_value);
                                    endif;
                                endif;
                            endif; ?>
                    </select>
            ...
            $this->print_metabox_field_begin_tag( $field, "metabox_field_{$field['id']}", "metabox_field mtop10" ); ?>
                ...
                <?php if ( $has_field_multiple ) : ?>
                    <div class="field_media_navigation mtop10">
                        <a class="field_media_widget_add" href="#">
                            <img class="vmiddle" src="<?php echo METABOX_DIRECTORY_IMAGES_URI . '/widget_add.png' ?>" 
                                    alt="widget_add" 
                                        ...
                                        <div class="inlineblock vmiddle">
                                            <?php if ( $has_name_box ) : ?>
                                                <div>
                                                    ...
                                                    <input type="text" 
                                                        ...
                                                        data-prefix="<?= ( $has_field_multiple ? $field_name . '[0]' : $field_name ) ?>"
                                                        data-name="[title]"
                                            ...
                                            <?php endif; ?>
                                                <div class="mtop5">
                                                    ...
                                                    <?php endif; ?>
                                                    <img src="<?= $field_media_thumbnail ? $field_media_thumbnail : METABOX_DIRECTORY_IMAGES_URI . '/empty-thumbnail.png' ?>" 
                                                        class="thumbnail_media_field_metabox vmiddle" />
                                                <input type="text" 
                                                        name="<?= ( $has_field_multiple ? $field_name . '[' . ( $count_field - 1 ) . ']' : $field_name ) . '[thumbnail]' ?>" 
                                                        data-prefix="<?= ( $has_field_multiple ? $field_name . '[' . ( $count_field - 1 ) . ']' : $field_name ) ?>"
                                                        data-name="[thumbnail]"
        ...
        function print_accordion_field_metabox_theme_callback( $post_id, $field ) {
            ...
            if ( is_array( $field_value ) &&
                ...
                <script type="text/javascript">
                    if ( typeof(accordion_indexes) === 'undefined' ) {
                        var accordion_indexes = [];
                    }
                    accordion_indexes['<?= "metabox_field_{$field['id']}" ?>'] = <?php echo $accordion_count; ?> // $field : field groupbox
                </script>
    <?php   endif;         
            $this->print_metabox_field_begin_tag( $field, "metabox_field_{$field['id']}", "metabox_field metabox_group_field mtop10" ); ?>                
                <div class="label">
                    <strong><?php echo $field['title'] ?></strong>
                </div> 
                <div class="desc mtop10">
                        ...
                        <div class="metabox-field-postbox-item metabox-accordion-item postbox closed accordion-template-item mtop20">
                            ...
                            <div class="metabox-section-body inside mtop20">
                                ...
                                <div class="accordion-groupbox mtop20">
                                    ...
                                    <button type="button" class="metabox-accordion-clone-button button button-default">
                                        <img class="vmiddle" width="20" src="<?php echo METABOX_DIRECTORY_IMAGES_URI . '/widget_clone.png' ?>" alt="metabox-clone-accordion" />
                                        <span class="vmiddle">Clone accordion</span>
                                    </button>
                                </div>
                                <!-- #accordion-groupbox -->
                            </div>
