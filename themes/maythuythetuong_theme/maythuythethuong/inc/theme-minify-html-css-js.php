<?php 
    ...
    define('CACHE_DIRECTORY_URI', 'http://' . $_SERVER['HTTP_HOST'] . '/wp-content/cache' );
    ...
    define('THEME_FONT_DIRECTORY_URI', 'http://' . $_SERVER['HTTP_HOST'] . '/wp-content/themes/' . $template_dir_name . '/fonts' );
    define('THEME_IMAGES_DIRECTORY_URI', 'http://' . $_SERVER['HTTP_HOST'] . '/wp-content/themes/' . $template_dir_name . '/images' );    
    ...
    class WP_HTML_Compression
    {
        ...
        public function combineStyleSheet( $html ) {
            ...
            // chèn tag link rel đến file css mới tạo ở trước tag </header>
            $stylesheet_tag = "<link rel='stylesheet' href='" . STYLESHEET_COMBINE_FILE_URI . "' type='text/css' />";

            $header_endtag_pos = strpos( $html, '</head>' );
            $html = substr( $html, 0, $header_endtag_pos ) . $stylesheet_tag . substr( $html, $header_endtag_pos );
        ...
        public function combineScripts( $html ) {
            ...
            // chèn tag <script type='text/javascript' đến file js mới tạo ở trước tag </header>      
            $script_tag = "<script type='text/javascript' src='" . JAVASCRIPT_COMBINE_FILE_URI . "' async></script>";

            $header_endtag_pos = strpos( $html, '</body>' );
            $html = substr( $html, 0, $header_endtag_pos ) . $script_tag . substr( $html, $header_endtag_pos );
