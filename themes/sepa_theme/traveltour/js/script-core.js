(function($){
	...
	function sm_countDown(e, start, end) {
        var now     = new Date().getTime();
        var iStart  = new Date(start).getTime();
        var iEnd    = new Date(end).getTime();
        if( iStart < now && now < iEnd ) {
            e.prevAll('svg').show();
            var x = setInterval(function() {
                var n     = new Date().getTime();
                var tmp   = iEnd - n;
                var d     = Math.floor(tmp / 86400000);
                var h     = Math.floor((tmp % 86400000) / 3600000);
                var m     = Math.floor((tmp % 3600000) / 60000);
                var s     = Math.floor((tmp % 60000) / 1000);
    ...
    $('.sm-handle-countdown').each(function(i,v) {
        var start   = $(v).attr('data-start');
        var end     = $(v).attr('data-end');
        var element = $(v).prevAll('.sm-countdown');
        sm_countDown($(element), start, end);
    });
