 ...
 */
	...
	if( is_tax('portfolio_category') || is_tax('portfolio_tag') && class_exists('gdlr_core_pb_element_portfolio') ){
		...
		$sidebar_type = traveltour_get_option('general', 'archive-portfolio-sidebar', 'none');
		$sidebar_left = traveltour_get_option('general', 'archive-portfolio-sidebar-left');
		$sidebar_right = traveltour_get_option('general', 'archive-portfolio-sidebar-right');		
	}else{
		$post_type = 'post';
