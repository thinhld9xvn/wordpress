    ...
    function update_slug_terms( $term_id, $taxonomy ) {
        ...
        if ( $exist_term && $term_id !== $exist_term->term_id ) :
            ...
            if ( is_numeric ( $str_endpart_url ) ) :
                $splices_url[ $ubound ] = intval( $str_endpart_url ) + 1;
...
function my_deregister_scripts_and_styles() {
  ...
  add_action('pre_get_posts', function( $query ) {
    $global_section_id = \Theme_Options\GLOBAL_FIELDS::GLOBAL_SECTION_ID;
    $global_options = get_option("{$global_section_id}_option_name");    
    if ( ! $query->is_admin ) :			
...
//
function woo_page_template_redirect() {
    if ( is_page_template( 'layout/page-account.php' ) && ! is_user_logged_in() ) {
        wp_redirect(get_login_page_url());
        exit();
    }
}
add_action( 'template_redirect', 'woo_page_template_redirect' );