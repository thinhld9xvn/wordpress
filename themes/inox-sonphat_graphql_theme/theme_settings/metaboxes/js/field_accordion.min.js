  ...
  var accordion = {
    ...
    ready: function ready() {
      ...
      $accordion_sortables.each(function (index, elem) {
        var accordion_title_field = $(elem).find('.accordion-template-item').find('.metabox-accordion-title').find('.title').attr('data-accordion-title-field');
        $(document).on('keyup', 'input[type=text][id*="' + accordion_title_field + '"]', accordion.on_accordion_field_title_keyup);
      });
      $(document).on('click', '.metabox-collapse-button', this.on_collapse_field_box).on('click', '.metabox-accordion-remove-button', this.on_accordion_remove_field).on('click', '.metabox-handle-title, .metabox-collapse-button', this.on_accordion_handle_click);
      $('.metabox-accordion-add-button').click(this.on_accordion_add_field);
      var $firstMetabox = $('#advanced-sortables .postbox:eq(0)').find('.metabox_groupbox_fields_sortables:eq(0)').find('.metabox-field-postbox-item:eq(0)');
      $firstMetabox.find('h2:eq(0)').click();
      $firstMetabox.removeClass('closed');
    },
    on_accordion_sortupdate: function on_accordion_sortupdate(event, ui) {
      var index = 0,
          id = '',
          name = '',
      ...
      $('.metabox-accordion-item:not(.accordion-template-item)').each(function () {
        ...
        $fields_has_name.each(function () {
          id = $(this).attr('id'), name = $(this).attr('name');
          var s = name.split(/\[\d\]/),
              ac_container = '',
              ac_field = '';
    ...
    on_collapse_field_box: function on_collapse_field_box(e) {
    ...
    on_accordion_add_field: function on_accordion_add_field(e) {
      ...
      $accordion_field.find('*[id*="-uo-"], *[name*="-uo-"]').each(function (index, elem) {
        var $obj = $(elem),
            id = $obj.attr('id'),
            name = $obj.attr('name');

        if (undefined !== id) {
          if ($obj.is('textarea') && $obj.hasClass('wp-editor-area')) {
      ...
      $accordion_field.removeClass('closed').appendTo($accordion_group_fields);
      var $textarea = $accordion_field.find('.wp-editor-wrap').find('textarea');
      $textarea.each(function (index, elem) {
        ...
        var qt_instance = quicktags({
          id: textarea_id
        }),
            t = setInterval(function () {
          if (_typeof(qt_instance) === 'object') {
            QTags._buttonsInit();

            $editor_toolbar.find('.wp-switch-editor.switch-html').click();
            clearInterval(t);
          }
        }, 200);
      });
    ...
    on_accordion_remove_field: function on_accordion_remove_field(e) {
      var $accordion = $(this).closest('.metabox-accordion-item'); // remove instance tinymce

      $accordion.find("textarea[class='wp-editor-area']").each(function (index, elem) {
        tinymce.get($(elem).attr('id')).remove();
      });
      $accordion.remove();
      accordion.accordion_index--;
    },
    on_accordion_field_title_keyup: function on_accordion_field_title_keyup(e) {
      var $accordion_item = $(this).closest('.metabox-accordion-item'),
          $accordion_title = $accordion_item.find('.metabox-accordion-title').find('.title'),
          text = $(this).val();
    ...
    on_accordion_handle_click: function on_accordion_handle_click(e) {
      var $obj = $(this);
      ...
      $parent.closest('.metabox-field-postbox-item').siblings().find('h2.metabox-handle-title').removeClass('markup');
      ...
      if ($parent.hasClass('closed')) {
        ...
        if ($c_parent.length > 0) {
          $c_parent.find('h2.metabox-handle-title').removeClass('markup');
