<?php 
	...
	function the_page_navigation( $class='' ) {
        ...
        if ( get_previous_posts_link() ) :
        	...
        	printf( '<li><a href="%s"><span class="fa fa-angle-left"></span></a></li>', esc_url( get_pagenum_link( $paged - 1 ) ) );
        ...
        if ( get_next_posts_link() ) :
       	 	...
       	 	printf( '<li><a href="%s"><span class="fa fa-angle-right"></span></a></li>', esc_url( get_pagenum_link( $paged + 1 ) ) );        	
	...
	function renderStyleBackgroundImage() {
		...
		if ( ! is_home() && ! is_front_page() ) :
			...
			if ( is_archive() || is_singular() ) :
				...
				$dichvu_post_type = \Archive\Services\SVGetPostTypeUtils::get();
				$uudai_post_type = \Archive\UuDai\UD_FIELDS::POST_TYPE;
				$tintuc_post_type = \Archive\News\NewsGetPostTypeUtils::get();	
				$galleries_post_type = \Archive\Galleries\GalleriesGetPostTypeUtils::get();	

				$url = '';

				if ( $post_type === $uudai_post_type || is_singular($uudai_post_type) ) :

					$url = \Archive\UuDai\UDGetBackgroundImageUtils::get();

				elseif ( $post_type === $tintuc_post_type || is_singular($tintuc_post_type) ) :

					$url = \Archive\News\NewsGetBackgroundImageUtils::get();
					
				elseif ( $post_type === $galleries_post_type || 
							is_singular($galleries_post_type) || 
								is_page_template('layout/page-galleries-layout.php') ) :

					$url = \Archive\Galleries\GalleriesGetBackgroundImageUtils::get();	

				elseif ( $post_type === $dichvu_post_type || is_singular($dichvu_post_type) ) :

					$url = \Archive\Services\SVGetBackgroundImageUtils::get();		
					
				else :

					echo "data-src='" . MY_THEME_DIR_URI . "/assets/images/banner.png'";
