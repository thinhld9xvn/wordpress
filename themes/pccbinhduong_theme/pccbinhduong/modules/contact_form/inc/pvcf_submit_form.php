<?php 
    ...
    function sb_pvcf_contact_form_submit_script() {
        ...
        wp_enqueue_style( 'pvcf-ajax-submit-form-stylesheet', PVCF_DIRECTORY_CSS . '/style.css' );
        wp_enqueue_script('pvcf-ajax-validate-submit-form', PVCF_DIRECTORY_JS . '/pvcf_validate.js', array('jquery'), 'v1.0.0' , true);
        wp_enqueue_script('pvcf-ajax-submit-form', PVCF_DIRECTORY_JS . '/pvcf_submit_form.js', array('jquery'), 'v1.0.0' , true);        
     ...
     function pvcf_send_mail( $form_data, $form_id ) {
        ...
        /*$email_name = $pvcf_email_options['contactform-email-name-id'];  
        $smtp_host = $pvcf_email_options['contactform-smtp-host-id'];
        $smtp_port = $pvcf_email_options['contactform-smtp-port-id'];
        $smtp_encryption = $pvcf_email_options['contactform-smtp-encryption-id'];
        $smtp_authentication_username = $pvcf_email_options['contactform-authentication-username-id'];
        $smtp_authentication_password = $pvcf_email_options['contactform-authentication-password-id'];*/
       ...
       if ( $result ) {
        ...
        //echo $email_name . ' - ' . $smtp_host . ' - ' . $smtp_port . ' - ' . $smtp_encryption . ' - ' . $smtp_authentication_username . ' - ' . $smtp_authentication_password;
        die();
