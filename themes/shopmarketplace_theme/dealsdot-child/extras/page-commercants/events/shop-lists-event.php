<?php
	...
	if ( isset( $_POST['btnCommercantsSm'] ) ) :

		$file_url = $_POST['txtExcelFile'];
		$txtAddrLists = $_POST['txtCommercantsAddrLists'];
			...
			if ( $xlsx = SimpleXLSX::parseData( $file_contents ) ) :
				...
				if ( $enseigne_idx !== false ) :
					...
					foreach ( $rows as $i => $row ) :
						...
						$numero = $numero_idx !== false ? $row[$numero_idx] : '';
						$adresse = $adresse_idx !== false ? $row[$adresse_idx] : '';
						$ville = $ville_idx !== false ? $row[$ville_idx] : '';
						$phone = $phone_idx !== false ? $row[$phone_idx] : '';
						$secteur = $secteur_idx !== false ? $row[$secteur_idx] : '';
						$code_postal = $code_postal_idx !== false ? $row[$code_postal_idx] : '';
						...
						$commercants_list .= sprintf("enseigne: %s\n" .
													  "numero: %s\n" .
													  "address: %s\n" .
													  "ville: %s\n" .
													  "phone: %s\n" .
													  ...
													  "code postal: %s\n" .
													  "email commerce: %s\n" .
													  "portable responsable: %s\n" .
													  "email responsable: %s\n" .
													  "site internet: %s\n" .
													  "page facebook: %s\n" .
													  "jours ouverture hiver : %s\n" .
													  "horaires hiver : %s\n" .
													  "jours fermeture hiver : %s\n" .
													  "jours ouverture été : %s\n" .
													  "horaires été : %s\n" .
													  "jours fermeture ete : %s\n" .
													  "particularites horaires : %s\n" .
													  "annuaire : %s\n" .
													  "masques recus : %s\n" .
													  "bail saisonnier  : %s\n",
													  $enseigne,
						...
						$commercants_list .= \Stores\STORE_DATA::STORE_DELIMITER . "\n";
