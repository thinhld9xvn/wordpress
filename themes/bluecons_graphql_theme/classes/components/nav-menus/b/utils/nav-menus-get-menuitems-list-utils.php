<?php 
    ...
    class NavMenusGetMenuItemsListUtils {   
        private static function get_afc_attributes($item) {
            $background = !empty($item->term) ? get_field(NAVMENUS_FIELDS::AFC_THUMBNAIL_FIELD, $item->term) : '';
            if (!empty($background)) {
        ...
        private static function get_class($item) {
            return !empty($item->classes) ? $item->classes[0] : '';
        }
        private static function populate_children($menu_array, $menu_item, $mega) {
            $children = array();                
            if (!empty($menu_array)){
                foreach ($menu_array as $k=>$m) { 
                    ...
                    if ($m->menu_item_parent == $menu_item->ID) {
                        //print_r($m);
                        extract(self::get_afc_attributes($m));
                        $children[$m->ID] = array();
                        $is_mega = self::get_class($m) === 'services';
                        ...
                        unset($menu_array[$k]);
                        $children[$m->ID][NAVMENUS_FIELDS::MEGA_GQL_FIELD] = $is_mega;
                        $children[$m->ID][NAVMENUS_FIELDS::CHILDRENS_GQL_FIELD] = self::populate_children($menu_array, $m, $mega);
                        $children[$m->ID][NAVMENUS_FIELDS::ORDER_GQL_FIELD] = $m->menu_order;
                        $children[$m->ID][NAVMENUS_FIELDS::BACKGROUND_GQL_FIELD] = $background;
                    }
                }
            };
            return $children;
        ...
        public static function get($menu_location) {
            ...
            foreach ($menu_array as $m) {
                if (empty($m->menu_item_parent)) {
                    ...
                    $menu[$m->ID][NAVMENUS_FIELDS::MEGA_GQL_FIELD] = $is_mega;              
                    $menu[$m->ID][NAVMENUS_FIELDS::CHILDRENS_GQL_FIELD] = self::populate_children($menu_array, $m, $is_mega);
                    $menu[$m->ID][NAVMENUS_FIELDS::ORDER_GQL_FIELD] = $m->menu_order;
                    $menu[$m->ID][NAVMENUS_FIELDS::BACKGROUND_GQL_FIELD] = $background;
                }
            }              
