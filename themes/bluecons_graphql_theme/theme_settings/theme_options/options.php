...
class MyThemeOptionPage {
     ...
     */       
     public function __construct() {
        include MY_THEME_DIR . '/options/theme_options.php';
    ...
    public function add_plugin_page() {
        ...
        add_menu_page(__( 'Theme Option', 'pitvietco' ), // Thay title của trang Theme Option
            ...
            'manage_options', 'theme-option', null, MY_THEME_DIR_URI . '/theme_settings/theme_options/theme_option.png' // Thay icon của bạn
            );           
         foreach( $sections as $section ) :
            ...
            $section_hook = add_submenu_page('theme-option', 
                                                'Theme Option - ' .  $section_title, 
                                                $section_title , 
                                                'manage_options', 
                                                $section_url, array(
                                                        &$this,
    ...
    public function print_field_media_type_callback($field, $section) {
            ...
            ob_start(); ?>
                <div class="media_thumbnail">
                    ...
                    <input id="<?php echo $field['id'] . '-input-id' ?>"
                            type="hidden" 
                            name="<?php echo $section['id'] . '_option_name[' . $field['id'] . '-id]' ?>" 
                            value="<?= isset( $this->options[ $field['id'] . '-id' ] ) ? 
                                            esc_attr( $this->options[ $field['id'] . '-id' ] ) : '' ?>" />
                </div>
    ...
    public function print_post_type_field_select_options($field, $section) {
       ...
       $args = array(
        ...
        foreach ( $post_types as $post_type ) :
           $sl_options .= sprintf('<option value="%1$s" %2$s>%3$s</option>', 
                               $post_type->name,
                               $post_type->name === $this->options[ $field['id'] . '-id' ] ? 'selected="selected"' : '',
...
function theme_option_load_style() {
    ...
    if ( false !== strpos( $request_uri, 'theme-option' ) ) :
        wp_enqueue_media();
        wp_enqueue_style( 'dealsdot-themeoption-style' , MY_THEME_DIR_URI . '/theme_settings/theme_options/css/style.min.css' );
        wp_enqueue_script( 'dealsdot-themeoption-field-media-js' , MY_THEME_DIR_URI . '/theme_settings/theme_options/js/field_media.min.js' );
        wp_enqueue_script( 'dealsdot-themeoption-field-editor-js' , MY_THEME_DIR_URI . '/theme_settings/theme_options/js/field_editor.min.js' );
        wp_enqueue_script( 'dealsdot-themeoption-typical-js' , MY_THEME_DIR_URI . '/theme_settings/theme_options/js/field_typical.min.js' );
        //$combine_admin_enqueue['stylesheet'][] = MY_THEME_DIR_URI . '/theme_settings/theme_options/css/style.min.css';
        //$combine_admin_enqueue['scripts'][] = MY_THEME_DIR_URI . '/theme_settings/theme_options/js/field_media.min.js';
        //$combine_admin_enqueue['scripts'][] = MY_THEME_DIR_URI . '/theme_settings/theme_options/js/field_editor.min.js';                
        //$combine_admin_enqueue['scripts'][] = MY_THEME_DIR_URI . '/theme_settings/theme_options/js/field_typical.min.js';                       
    endif;
}
