<?php 
	...
	class qTranslate_db {
				...
				if ( 0 == sizeof( $result ) ) :
					...
					$mydb->query(				
						...
						"
						  ...
						  ALTER TABLE $tbl_terms ADD COLUMN qtranslate_term_langcode VARCHAR(2) NOT NULL DEFAULT 'vi'
				...
				if ( 0 == sizeof( $result ) ) :
					...
					$mydb->query(
						...
						"
						  ...
						  ALTER TABLE $tbl_terms ADD COLUMN qtranslate_term_id_primary MEDIUMINT(9) NOT NULL DEFAULT 'vi'
				...
				if ( 0 == sizeof( $result ) ) :
					...
					$mydb->query(				
						...
						"
						  ...
						  ALTER TABLE $tbl_posts ADD COLUMN qtranslate_post_langcode VARCHAR(2) NOT NULL DEFAULT 'vi'
				...
				if ( 0 == sizeof( $result ) ) :
					...
					$mydb->query(				
						...
						"
						  ...
						  ALTER TABLE $tbl_posts ADD COLUMN qtranslate_post_id_primary MEDIUMINT(9) NOT NULL DEFUATL 'vi'
		...
		function set_all_contents_to_lang_default( $def_langcode ) {
			...
			//$tbl_taxonomy = $mydb->prefix . 'term_taxonomy';
			...
			$set_contents_def_callback = function( $db, $arr_tables, $lcode ) {				
				...
				$db->query(
					...
					"
						UPDATE 
							...
							{$arr_tables['terms']} as t

						SET 
							t.qtranslate_term_langcode = '$lcode'

						WHERE t.qtranslate_term_langcode = ''
				...
				/*echo "
						...
						WHERE qtranslate_post_langcode = ''
		...
		protected function check_and_create_column_lang_string( $langcode ) {
			...
			if ( 0 == sizeof( $result ) ) :
				...
				$mydb->query( 
					...
					"
						ALTER TABLE $tbl_qtranslate_strings ADD COLUMN string_value_$langcode VARCHAR(100) NOT NULL DEFAULT 'vi'
		...
		/* 
			lấy dữ liệu term foreign
			@params:
				@term_id : term primary id
				@p_langcode : term langcode

		*/

		function get_term_foreign( $term_id, $t_langcode, $taxonomy = 'category' ) {
			...
			$tbl_terms = $mydb->prefix . 'terms';
			$tbl_term_taxonomy = $mydb->prefix . 'term_taxonomy';			
			...
			return $mydb->get_row(
				...
				"
					...
					SELECT 
						...
						{$tbl_terms}.term_id, name, slug, {$tbl_term_taxonomy}.taxonomy
					...
					WHERE
						...
						qtranslate_term_id_primary = {$term_id} AND

						{$tbl_term_taxonomy}.taxonomy = '{$taxonomy}'
		...
		function unlink_term_foreign( $term_id, $t_langcode, $taxonomy = 'category' ) {
		...
		/* 
			lấy dữ liệu post foreign
			@params:
				@post_id : primary post id
				@p_langcode : post langcode

		*/

		function get_post_foreign( $post_id, $p_langcode, $post_type = 'post' ) {
			...
			return $mydb->get_row(
				...
				"
					...
					SELECT 
						...
						id, post_title, post_name, post_content, qtranslate_post_id_primary
					...
					WHERE
						...
						qtranslate_post_id_primary = {$post_id} AND 
		...
		function unlink_post_foreign( $post_id, $p_langcode, $post_type = 'post' ) {
