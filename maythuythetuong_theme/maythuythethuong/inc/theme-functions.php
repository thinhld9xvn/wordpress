<?php 
	...
	add_action('pre_get_posts', function( $query ) {
		...
		if ( ! $query->is_admin ) :
			...
			if ( $main_query_args['nopaging'] ) :

				define('QUERY_POSTS_PER_PAGE', intval( $main_query_args['posts_per_page'] ) );

				$query->set( "posts_per_page", QUERY_POSTS_PER_PAGE );
				...
				if ( $query->is_tax ) :
					...
					define('QUERY_POSTS_PER_PAGE', intval( $global_options["global-pagenumber-post-type-$post_type-id"] ) );
				...
				elseif ( $query->is_category ) :
					...
					define('QUERY_POSTS_PER_PAGE', intval( $global_options["global-pagenumber-category-$category-id"] ) );
