...
class MyThemeOptionPage {
    ...
    public function add_plugin_page() {
        ...
        add_menu_page(__( 'Theme Option', 'pitvietco' ), // Thay title của trang Theme Option
            ...
            'manage_options', 'theme-option', null, get_stylesheet_directory_uri('stylesheet_directory') . '/theme_settings/theme_options/theme_option.png' // Thay icon của bạn
            );           
    ...
    public function print_taxonomy_field_select_options($field, $section) {
        ...
        if ( $taxonomies ) :
          ...
          foreach ( $taxonomies as $taxonomy ) :
            ...
            $sl_options .= sprintf('<option value="%1$s" %2$s>%3$s</option>', 
                                     $taxonomy->name,
                                     $taxonomy->name === $this->options[ $field['id'] . '-id' ] ? 'selected="selected"' : '',
                                     $taxonomy->label
                            );
...
if ( is_admin() ) :
    ...
    function theme_option_load_style_script() {

        $page = $_GET['page'];
        
        if ( false !== strpos( $page, 'theme-option' ) ) :
            
          wp_enqueue_media();

        	wp_enqueue_style( 'my-theme-option-style', get_template_directory_uri() . '/theme_settings/theme_options/css/style.min.css' );	        	
        	wp_enqueue_script('my-theme-option-jquery-field-options',  get_template_directory_uri() . '/theme_settings/theme_options/js/field_options.min.js', array('jquery'), '1.0.0', true);        	        	
        	
        endif;
        
    }
    
   add_action( 'admin_enqueue_scripts', 'theme_option_load_style_script' );
