	...
	function the_breadcrumbs( $breadcumbs_id, $breadcumbs_class, $home_text, $d ) {
		...
		if ( ! is_home() && ! is_front_page() || is_paged() ) {
			...
			if ( get_query_var('paged') ) {
				...
				if ( is_category() || is_tax() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ' (';
	...
	add_action('pre_get_posts', function( $query ) {
		...
		if ( ! $query->is_admin ) :
			...
			if ( $main_query_args['no_paging'] ) :				
				...
				$query->set( "posts_per_page", intval( $main_query_args['posts_per_page'] ) );
				...
				if ( $query->is_tax ) :
					...
					$query->set( "posts_per_page", intval( $global_options["global-pagenumber-post-type-$post_type-id"] ) );				
				...
				elseif ( $query->is_category ) :
					...
					$query->set( "posts_per_page", intval( $global_options["global-pagenumber-category-$category-id"] ) );
